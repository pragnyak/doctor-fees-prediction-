# -*- coding: utf-8 -*-
"""
Created on Sat Jul  6 12:20:15 2019
...
for tup in all_qua_test:
    test_qua.append(str(tup[0]).strip())
...
for x in dataset["Qualification"].values:
    for each in x:
        each = each.strip()
        if each in all_Qual_train:
            all_Qual_train[each]+=1
        else:
            all_Qual_train[each]=1
all_qua_train = sorted(all_Qual_train.items(),key=lambda x:x[1],reverse=True)
train_qua =[]
for tup in all_qua_train:
    train_qua.append(str(tup[0]).strip())

final_qua=test_qua+train_qua
final_qua=list(np.unique(np.array(final_qua)))
